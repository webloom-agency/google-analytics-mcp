...
class OAuthConfig:
    ...
    def __init__(self):
        self.base_uri = os.getenv("GA4_MCP_BASE_URI") or os.getenv("GSC_MCP_BASE_URI") or "http://localhost"
        self.port = int(os.getenv("PORT", os.getenv("GA4_MCP_PORT", os.getenv("GSC_MCP_PORT", "8000"))))
        self.base_url = f"{self.base_uri}:{self.port}"

        self.external_url = os.getenv("GA4_EXTERNAL_URL") or os.getenv("GSC_EXTERNAL_URL")
    ...
    def _get_redirect_uri(self) -> str:
        explicit_uri = os.getenv("GOOGLE_OAUTH_REDIRECT_URI") or os.getenv("GA4_OAUTH_REDIRECT_URI") or os.getenv("GSC_OAUTH_REDIRECT_URI")
        if explicit_uri:
            return explicit_uri
