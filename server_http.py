...
def _create_app():
    ...
    if MCP_ENABLE_OAUTH21 and _auth_provider:
        # Parity with the production mcp-gsc server: serve FastMCP's default
        # OAuth metadata unchanged (no Protected Resource Metadata override).
        # The _McpAuthDiagnostic wrapper is a transparent pass-through that only
        # logs whether /mcp requests carry an Authorization header.
